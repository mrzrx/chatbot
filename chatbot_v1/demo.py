...
max_token_id = wordToken.load_file_list('../corpus/corpus_xhj_10000.txt', min_freq)
num_encoder_symbols = max_token_id + 4 + 1
num_decoder_symbols = max_token_id + 4 + 1
print('min_freq=', min_freq, 'vocab_size=', num_encoder_symbols)
...
def get_train_set():
    ...
    train_set = []
    with open('../corpus/corpus_xhj_10000.txt', 'r', encoding='UTF-8') as corpus_file:
        while True:
...
def train():
    ...
    with tf.Session() as sess:
        ...
        for step in range(0,100000):
            ...
            if step % 100 == 0:
                print(time.ctime(), 'step=', step, 'loss=', loss_ret, 'learning_rate=', learning_rate.eval())
                # 模型持久化
                saver.save(sess, './model/demo', global_step=step)
            if step % 100 == 0:
...
def predict():
 ...
 #       saver.restore(sess, './model/v2/demo')
        ckpt=tf.train.get_checkpoint_state("./model/")
        if ckpt and ckpt.model_checkpoint_path:
            saver.restore(sess,ckpt.model_checkpoint_path)
        else:
